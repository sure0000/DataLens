...
> **产品/UI 演进：** 见 [本体三层架构与 UI 优化方案](./本体三层架构与UI优化.md)。  
> **资料到产出物映射：** 见 [企业资料到本体产出物映射](./企业资料到本体产出物映射.md)。
...
所有抽取通过 LLM 完成，用户无需任何标注或配置。实际编排见 `backend/services/extraction/orchestrator.py`（9 步：term → metric → dimension → rule → relation → hierarchy → lineage → join → domain_term）。以下按概念阶段介绍：
