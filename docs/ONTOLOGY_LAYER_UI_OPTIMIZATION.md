...
| 场景 | 入口 | 默认 Tab |
|------|------|----------|
| 只读浏览 | 侧栏「本体浏览」→ `/ontology`；知识库内「本体浏览」 | 总览（`overview`） |
| 建模治理 | 侧栏「本体建模」→ `/ontology?tab=governance` | 清洗治理 |
| 专家调试 | 工作台「专家」Tab（末位） | SPARQL / 三元组 / TTL |

可选深链：`/ontology?kb={id}&tab=semantics` 选中知识库并打开对应 Tab。
...
| 场景入口（浏览/建模/专家 Tab） | ✅ | 已移除人员角色切换；侧栏「本体浏览」「本体建模」+ `?tab=` |
