    ...
    >
      ...
      {matched && mappings.length > 0 ? (
        <div className="mt-2">
          ...
          <ol className="mt-2 list-decimal space-y-2.5 pl-4 text-app-secondary">
            {mappings.map((row, idx) => (
              <li key={`${row.target_label}-${idx}`}>
                <p>{row.description || formatMappingFallback(row)}</p>
                {row.physical_tables ? (
                  <p className="mt-0.5 text-app-muted">
                    物理表：<span className="text-app-primary">{row.physical_tables}</span>
                  </p>
                ) : null}
              </li>
            ))}
