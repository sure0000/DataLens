...
export default function OntologyLayerDetailPanel({
  ...
  return (
    <div className="app-card p-4 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 className="text-sm font-semibold text-app-primary">
            {layerLabel ?? data?.label ?? layerKey}
          </h3>
      ...
      {loading && <p className="text-sm text-app-muted py-4">加载明细…</p>}

      {error && !loading && <p className="text-sm text-app-danger">{error}</p>}

      {!loading && !error && total === 0 && (
        <p className="text-sm text-app-muted py-4">本层暂无数据。</p>
      )}
              ...
              <tbody>
                {filteredItems.length === 0 ? (
                  ...
                  filteredItems.map((row, i) => (
                    <tr
                      key={`${row.s ?? i}-${i}`}
                      className="border-b border-app-border last:border-0 hover:bg-app-hover"
                    >
                      {columns.map((col) => (
                        <td
                          key={col.key}
                          className={`px-3 py-2 text-app-primary ${
                            col.wrap ? "max-w-[420px] whitespace-pre-wrap break-words align-top" : "max-w-[240px] truncate"
                          }`}
